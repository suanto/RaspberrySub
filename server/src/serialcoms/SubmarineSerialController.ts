...
/*
    ...
    12 = DISARM
*/
export class SubmarineSerialController implements ISubmarineController {
    private cmdSender : ICommandSender;
    private id : number = 1;

    constructor(sw : ICommandSender) {
        if (!sw) throw "SubmarineSerialController: sw not set"
        this.cmdSender = sw;
    }
    ...
    throttle(power: number): void {
        this.sendCommand("01", power);
    }
    yaw(power: number): void {
        this.sendCommand("02", power);
    }
    pitch(power: number): void {
        this.sendCommand("03", power);
    }
    roll(power: number): void {
        this.sendCommand("04", power);
    }
    activate_light(light: number): void {
        this.sendCommand("06", light);
    }
    change_light_intensity(intensity: number): void {
        this.sendCommand("05", intensity);
    }
    heartbeat(i: number): void {
        this.sendCommand("09", i);
    }
    powerfactor(factor: number) {
        this.sendCommand("10", factor);
    }

    private sendCommand(command : string, value? : number) {
        ...
        if (this.cmdSender) {
            const cmd = 
                "|" + this.id++ + 
                "|" + command + 
                "|" + (value ? this.parseValue(value) : "000000");
            crc.update(cmd);
            this.cmdSender.sendCommand(cmd + "|" + crc.finalize());
        }
    }

    private parseValue(value : number) : string {
        const postFix = ".000000"
        const valueString = value.toString();
        return valueString + postFix.substr(0,  6 - valueString.length);

    
        // const template = "000000";
        // const valueString = value.toString().substring(0,7);
        // return template.substr(0, 6 - valueString.length) + valueString;
    }
